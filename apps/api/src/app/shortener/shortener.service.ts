...
import { Link } from '@shortwarden/prisma';
...
export class ShortenerService {
  ...
  createDbUrl = async (
    ...
    const [_, createdLink] = await Promise.all([
      this.usageService.incrementLinksCount(user.id),
  ...
  getLinkFromDb = async (key: string): Promise<LinkValue | null> => {
    ...
    this.addLinkToCache(
      key,
      {
        ...
        utm: link.utm as Record<string, string>,
      },
      expirationTime
    );
